...
## Pie Charts
Function coord_polar() is used to produce a pie chart, which is just a stacked bar chart.
```{r pie chart}
...
- geom_polygon(): add polygon
- geom_path(): connect observations in original order
- geom_ribbon(): add ribbons
- geom_curve(): add curves
- geom_rect(): add 2d rectangles
- geom_segment(): add a single line segments. 

Customize the plot: alpha, color, fill, linetype, size
```{r paramters}
require(maps)
...
# color, size, face
# theme() and element_text() can be used, element_blank() hides the labels
...
- facet_grid(supp ~ .): two rows
- facet_grid(. ~ supp): two columns
- facet_grid(dose ~ supp): two columns and three rows
- facet_grid( ~ fl): Place facet side by sie into a rectangular layout
...
# The R package survminer contains the function ggsurvplot() for easily drawing beautiful and ready-to-publish survival curves using ggplot2. ggsurvplot() includes also some options for displaying the p-value and the ‘number at risk’ table, under the survival curves. 
require(survminer)
