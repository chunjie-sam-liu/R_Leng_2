...
# ##################################
hsa_mirna_feature_mitochondrion <- 
  hsa_mirna_feature %>% 
  mutate(subloc = ifelse(subloc %in% c("Mitochondrion"), "Mitochondrion", "ExtraMitochondrion")) %>% 
  as.data.frame()

hsa_mirna_cell_task <- 
  makeClassifTask(
    id = "hsa_mirna", 
    data = hsa_mirna_feature_mitochondrion, 
    target = "subloc"
  )
...
ggsave(filename = "fig_10_hsa_seed_learner_comparison_1_mitochondrion_mmce.png", path = human_path_mirna, device = "png")
...
ggsave(filename = "fig_10_hsa_seed_learner_comparison_2_mitochondrion_auc.png", path = human_path_mirna, device = "png")
